...
storeLocator.Panel.prototype.idle_=function(a){this.center_?a.getBounds().contains(this.center_)||(this.center_=a.getCenter(),this.listenForStoresUpdate_()):this.center_=a.getCenter()};storeLocator.Panel.NO_STORES_HTML_='\x3cli class\x3d"no-stores"\x3eNão há lojas nesta área.\x3c/li\x3e';storeLocator.Panel.NO_STORES_IN_VIEW_HTML_='\x3cli class\x3d"no-stores"\x3eNão há lojas nesta área. No entanto , as lojas mais próximas a você estão listados abaixo.\x3c/li\x3e';
...
storeLocator.Store.prototype.getInfoWindowContent=function(){if(!this.content_){var a=['\x3cdiv class\x3d"infowindow-store"\x3e'];a.push(this.generateFieldsHTML_(["title","address","tel","hours"]));a.push("\x3c/div\x3e");this.content_=a.join("")}return this.content_};
storeLocator.Store.prototype.getInfoWindowContentList=function(){if(!this.content_){var a=['\x3cdiv class\x3d"store"\x3e'];a.push(this.generateFieldsHTML_(["title","address","phone","misc","web"]));a.push(this.generateFeaturesHTML_());a.push("\x3c/div\x3e");this.content_=a.join("")}return this.content_};storeLocator.Store.prototype.getInfoPanelContent=function(){return this.getInfoWindowContentList()};storeLocator.Store.infoPanelCache_={};
 ...
 * @private
 */
LojasPH.prototype.FEATURES_ = new storeLocator.FeatureSet(
  new storeLocator.Feature('balcaoviagem-sim', 'Balcão pra viagem'),
  new storeLocator.Feature('cadeirante-sim', 'Cadeirante'),
  new storeLocator.Feature('cafe-sim', 'Café'),
  new storeLocator.Feature('delivery-sim', 'Delivery'),
  new storeLocator.Feature('estacionamento-sim', 'Estacionamento'),
  new storeLocator.Feature('express-sim', 'Express'),
  new storeLocator.Feature('restaurante-sim', 'Restaurante')
);

/**
 * @return {!storeLocator.FeatureSet}
 */
LojasPH.prototype.getFeatures = function() {
...
LojasPH.prototype.parse_ = function(csv) {
  var lojas = [];
  var rows = csv.split('\n');
  ...
  for (var i = 1, row; row = rows[i]; i++) {
	...
	features.add(this.FEATURES_.getById('balcaoviagem-' + row.BalcaoViagem));
	features.add(this.FEATURES_.getById('cadeirante-' + row.Cadeirante));
	features.add(this.FEATURES_.getById('cafe-' + row.Cafe));
	features.add(this.FEATURES_.getById('delivery-' + row.Delivery));
	features.add(this.FEATURES_.getById('estacionamento-' + row.Estacionamento));
	features.add(this.FEATURES_.getById('express-' + row.Express));
	features.add(this.FEATURES_.getById('restaurante-' + row.Restaurante));
	
	estadoSelect[i] = row.Estado;
	cidadeSelect[i] = row.Cidade;
    var posicao = new google.maps.LatLng(row.Latitude, row.Longitude);
    var endereco = this.join_([row.Endereco, row.Numero, row.Complemento, row.Bairro, row.CEP, row.Cidade, row.Estado], ', ','<br>');
    var loja = new storeLocator.Store(row.id, posicao, features, {
      title: row.Nome,
      address: this.join_([endereco], '<br>'),
      hours: row.Horario,
      tel: row.Telefone,
      bviagem: row.BalcaoViagem,
    ...
    /* Descrição das lojas painel */
    loja.getInfoWindowContentList = function() {
		var detalhes = this.getDetails();
		var listaAtendimento = '';
		
			if(detalhes.bviagem){
				listaAtendimento = listaAtendimento + '<li>Balcão pra viagem</li>';
			}
			if(detalhes.cafe){
				listaAtendimento = listaAtendimento + '<li>Café da manhã</li>';
			}
			if(detalhes.delivery){
				listaAtendimento = listaAtendimento + '<li>Delivery</li>';
			}
			if(detalhes.express){
				listaAtendimento = listaAtendimento + '<li>Express</li>';
			}
			if(detalhes.restaurante){
				listaAtendimento = listaAtendimento + '<li>Restaurante</li>';
			}
			
		var headerLoja, addressLoja, telLoja, atendimentoLoja, lojaConteudo, listaAtendimento;
			
		if(detalhes.title){lojaConteudo = '<div class="loc-name">'+detalhes.title+'</div>';}
		if(detalhes.address){lojaConteudo = lojaConteudo + '<div class="loc-addr">'+detalhes.address+'</div>';}
		if(detalhes.hours){lojaConteudo = lojaConteudo + '<div class="time-addr"><span>'+detalhes.hours+'</span></div>';}
		if(detalhes.tel){lojaConteudo = lojaConteudo + '<div class="tel-addr"><span>'+detalhes.tel+'</span></div>';}
		if(listaAtendimento){lojaConteudo = lojaConteudo + '<div class="atendimento-addr"><ul>'+listaAtendimento+'</ul></div>';}
		if(detalhes.cadeirante){lojaConteudo = lojaConteudo + '<div class="cadeirante-addr"><span>Serviço para cadeirante</span></div>';}
		if(detalhes.estacionamento){lojaConteudo = lojaConteudo + '<div class="estacionamento-addr"><span>Estacionamento</span></div>';}
	...
	estadoSelect = uniq(estadoSelect);
	cidadeSelect = uniq(cidadeSelect);
	
	for(var item in estadoSelect){
	  $('<option value="'+estadoSelect[item]+'">'+estadoSelect[item]+'</option>').appendTo('#estado');
	}
	...
	/* Marca labels ativos nos filtros */
	$('.feature-filter label').click(function() {
		$('.feature-filter label input[type=radio], label input[type=checkbox]').click(function() {
		  $('.feature-filter label:has(input:checked)').addClass('ativo');
		  $('.feature-filter label:has(input:not(:checked))').removeClass('ativo');
		});
	});
 ...
 * @return {string}
 */
LojasPH.prototype.join_ = function(arr, sep) {
  var parts = [];
 ...
 * @return {Array.<string>}
 */
LojasPH.prototype.parseRow_ = function(row) {
  // Strip leading quote.
  if (row.charAt(0) == '"') {
    row = row.substring(1);
  }
 ...
 * @return {Object}
 */
LojasPH.prototype.toObject_ = function(headings, row) {
  var result = {};
...
var map, panelDiv, view, data, valor, valorFranquia;
...
google.maps.event.addDomListener(window, 'load', function() {
	...
	/* Cria os markers personalizados */
...
/* Seta o centro do mapa ao alterar o estado ou cidade */
var trocaLocal = function(valor){
	geocoder = new google.maps.Geocoder();
	geocoder.geocode({address: valor + ' - Brasil'}, function(results, status) {
		if (status == google.maps.GeocoderStatus.OK) {
			
			/* Fecha janela de info da loja selecionada */
			$('.gm-style-iw').next().click();
	        $('.error-msg').remove();
...
var filtroSelect = function(){
	var a = new Array();
	$('#cidade').children("option").each(function(x){
		test = false;
		b = a[x] = $(this).text();
